...
export default function MetaTagsPage() {
  ...
  // Fingerprint + free use tracking
  const [visitorId, setVisitorId] = useState<string | null>(null);
  const [freeUsed, setFreeUsed] = useState<boolean | null>(null);
  const [gateLoading, setGateLoading] = useState(true);
  const fpLoaded = useRef(false);

  // Load FingerprintJS and check free trial status
  useEffect(() => {
    if (fpLoaded.current) return;
    fpLoaded.current = true;
    ...
    const init = async () => {
      try {
        ...
        // Also check cookie
        const cookieUsed = document.cookie.includes("meta_tags_free_used=1");
        ...
        const res = await fetch("/api/free-trial", {
          ...
          body: JSON.stringify({ visitorId: vid, action: "check" }),
        });
        const data = await res.json();
        setFreeUsed(data.used);
      } catch {
        // Fallback: just check cookie
        setFreeUsed(document.cookie.includes("meta_tags_free_used=1"));
      }
      setGateLoading(false);
    };
    init();
  ...
  // Show auth gate if free use is exhausted and not logged in
  if (needsAuth) {
    return (
      <div style={{ maxWidth: 520, margin: "0 auto", padding: "80px 24px", textAlign: "center" }}>
        <div style={{ fontSize: 48, marginBottom: 16 }}>🔒</div>
        <h2 style={{ fontSize: 22, fontWeight: 800, margin: "0 0 8px", fontFamily: S.sans }}>Free Trial Used</h2>
        <p style={{ fontSize: 14, color: S.dim, margin: "0 0 8px", lineHeight: 1.6 }}>
          You&apos;ve used your free AI meta tag generation.
        </p>
        <p style={{ fontSize: 13, color: "#475569", margin: "0 0 24px", lineHeight: 1.6 }}>
          Sign in with Google and purchase credits to keep generating optimized meta tags with AI.
        </p>
  ...
  return (
    <div style={{ maxWidth: 720, margin: "0 auto", padding: "32px 24px 48px" }}>
      ...
      {/* Free trial badge */}
      {canUseFreely && !isLoggedIn && (
        <div style={{
          padding: "10px 16px", marginBottom: 16, borderRadius: 8,
          background: "rgba(34,197,94,0.08)", border: "1px solid rgba(34,197,94,0.2)",
          display: "flex", alignItems: "center", gap: 8, fontSize: 13, color: S.green,
        }}>
          <span>🎁</span>
          <span style={{ fontWeight: 600 }}>1 free generation available</span>
          <span style={{ color: S.dim, fontSize: 12 }}>— no sign-in required</span>
        </div>
      )}

      {/* Credits notice for logged-in users */}
      {needsCredits && (
        <div style={{
          padding: "10px 16px", marginBottom: 16, borderRadius: 8,
          background: "rgba(139,92,246,0.06)", border: "1px solid rgba(139,92,246,0.15)",
          display: "flex", alignItems: "center", gap: 8, fontSize: 13, color: S.purple,
        }}>
          <span>⚡</span>
          <span style={{ fontWeight: 600 }}>1 credit per generation</span>
          <Link href="/pricing" style={{ marginLeft: "auto", color: S.accent, fontSize: 12, fontWeight: 600, textDecoration: "none" }}>Buy Credits</Link>
        </div>
      )}
      ...
      <div style={{ padding: 20, background: "rgba(139,92,246,0.06)", border: `1px solid rgba(139,92,246,0.15)`, borderRadius: 12, marginBottom: 28 }}>
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: 12 }}>
          ...
          <button onClick={handleGenerate} disabled={generating}
            ...
            {generating ? "🔄 Reading page & generating..." : canUseFreely && !isLoggedIn ? "🎁 Generate Free" : "🚀 Generate with AI (1 credit)"}
          </button>
