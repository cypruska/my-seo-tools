...
const S = {
  ...
  border: "#1e2a42", focus: "#3b82f6", text: "#e2e8f0", dim: "#64748b",
  accent: "#3b82f6", green: "#22c55e", red: "#ef4444", yellow: "#eab308",
  mono: "'IBM Plex Mono', monospace", sans: "'DM Sans', system-ui, sans-serif",
};

const label: React.CSSProperties = {
  fontSize: 12, fontWeight: 700, color: S.dim, fontFamily: S.sans,
  letterSpacing: "0.05em", textTransform: "uppercase", marginBottom: 8, display: "block",
};
const input: React.CSSProperties = {
  width: "100%", boxSizing: "border-box", padding: "10px 12px",
  fontFamily: S.mono, fontSize: 13, background: S.surfaceAlt,
  border: `1px solid ${S.border}`, borderRadius: 6, color: S.text, outline: "none",
};
const btn: React.CSSProperties = {
  padding: "6px 14px", fontSize: 12, fontWeight: 600, fontFamily: S.sans,
  borderRadius: 6, border: `1px solid ${S.border}`, background: S.surfaceAlt,
...
function test(parsed: ParseResult, ua: string, path: string): TestResult {
  const uaL = ua.toLowerCase();
  let bestG: Group | null = null, bestS = -1;
  parsed.groups.forEach(g => g.agents.forEach(a => {
    if (a === "*") { if (bestS < 0) { bestG = g; bestS = 0; } }
    else if (uaL.includes(a) || a.includes(uaL)) { if (a.length > bestS) { bestG = g; bestS = a.length; } }
  }));
  if (!bestG || (bestG as Group).rules.length === 0)
    return { allowed: true, reason: "No matching rules — allowed by default.", matchedRule: null, group: bestG };
  let bestR: Rule | null = null, bestL = -1;
  (bestG as Group).rules.forEach(r => {
    if (r.type === "disallow" && r.path === "") return;
    if (!r.path) return;
    if (pathRe(r.path).test(path)) {
      const sp = r.path.replace(/\*/g, "").length;
      if (sp > bestL) { bestL = sp; bestR = r; }
      else if (sp === bestL && r.type === "allow") bestR = r;
    }
  });
...
function Validator() {
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: 20, padding: "24px 0" }}>
      ...
      <button onClick={() => setResult(test(parsed, ua, tp))} style={{
...
function Builder() {
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: 20, padding: "24px 0" }}>
      ...
      <button onClick={() => setGroups([...groups, { agent: "*", rules: [] }])}
        style={{ ...btn, borderColor: S.accent, color: S.accent }}>+ Add Bot Group</button>
      <div>
